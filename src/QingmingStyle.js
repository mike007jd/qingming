...
export class QingmingStyle {
  ...
  convertMaterial(source, art = {}) {
    ...
    target.defines={...target.defines,...source.defines};
    target.onBeforeCompile=source.onBeforeCompile;
    const sourceKey=source.customProgramCacheKey();target.customProgramCacheKey=()=>sourceKey;
    return this._decorate(target,art);
  }

  setPerformanceMode(enabled){this.performanceMode=enabled;this.updatePerformanceMode();}

  updatePerformanceMode(){
    const full=!!this.performanceMode&&this.settings.enabled===1;
    if(full===this.performancePaint)return;this.performancePaint=full;
    for(const material of this.materials)material.needsUpdate=true;
  }

  decorateMaterial(material, art = {}) { return this._decorate(material, art); }
  ...
  _decorate(material, {
    ...
    material.onBeforeCompile = (shader,renderer) => {
      ...
      // At 100% scroll paint, shade replaces the entire PBR lighting result.
      // Keep normal/albedo/shadows; omit only the lighting that cannot contribute.
      if(this.performancePaint&&material.isMeshStandardMaterial){
        shader.fragmentShader=shader.fragmentShader.replace('#include <lights_fragment_begin>',
          'vec3 geometryNormal=normal; vec3 geometryViewDir=(isOrthographic)?vec3(0,0,1):normalize(vViewPosition);')
          .replace('#include <lights_fragment_maps>','').replace('#include <lights_fragment_end>','');
      }
      previousHook.call(material,shader,renderer);
    };
    material.customProgramCacheKey = () => 'qingming-model-1.2:'+Number(this.performancePaint)+':'+previousKey;
