// Frozen before the unused-outline-pass optimization.
...
export class QingmingPass {
  ...
  _normalMaterial(source) {
    ...
    if (!source.isMeshStandardMaterial && !source.isMeshPhongMaterial && !source.isMeshLambertMaterial && !source.isMeshBasicMaterial) {
      ...
      material.customProgramCacheKey=()=>`qingming-normal-1.1-${alpha}:${source.customProgramCacheKey()}`;
    }
  ...
  render(scene,camera,outputTarget=null,{colorTexture=null}={}) {
