export async function loadCity(progress=()=>{}){
  ...
  const response=await fetch('./public/runtime/'+(soa?manifest.geometry.file:'city.bin.gz'));if(!response.ok)throw new Error('Could not load city geometry');
  const total=+response.headers.get('Content-Length')||(soa?102873728:29594434);const chunks=[];let received=0;
